...
# Select rows in 0.67G with Adj.P < 0.05 and Log2fc > 1
G0_cols=c(1, 2, grep("FLT_0.67G.v.HGC.", colnames(data)))
...
# Write output file
output_file=paste(args[2], "_0.67GvHGC_SigDEdata.txt", sep="")
output_file=paste(args[2], "_0.67G_SigLogandQ.txt", sep="")
write.table(M, file=output_file, quote=FALSE, sep="\t", row.names=FALSE, col.names=TRUE)

output_file=paste(args[2], "_0.67G_All.txt", sep="")
...
output_file=paste(args[2], "_0.67GvsHGC_SigLogTable.txt", sep="")
